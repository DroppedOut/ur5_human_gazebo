...
//ros communications:
  ...
  int main(int argc, char **argv) {
      ...
      int k;
      nh.getParam(ros::this_node::getName()+"/rand", k);
      ...
      std::string box_path;
      bool get_red_path;
      get_red_path = nh.getParam(ros::this_node::getName()+"/box_path", box_path);

      if (!(get_red_path)){
          return 0;}
          else{ROS_INFO_STREAM(box_path << " has been extracted");
}

      std::ifstream red_inXml(box_path.c_str());
      std::stringstream red_strStream;
      std::string red_xmlStr;

      /*red_inXml.open(box_path.c_str());*/
     ...
     // ROS_INFO_STREAM("urdf: \n" <<red_xmlStr);
      ...
      spawn_model_req.initial_pose.position.z = 1.02;
      ...
      for(int i=0;i<5;i++){
          ...
          spawn_model_req.initial_pose.position.y = (float)((rand()%40-10)*0.01);  
          spawn_model_req.initial_pose.position.x = (float)((rand()%30-5)*0.01); //(float)((rand()%70-40)*0.01); 
          xs[i]=spawn_model_req.initial_pose.position.x;
          ys[i]=spawn_model_req.initial_pose.position.y;
          for(int j=0;j<i;j++)
          if((fabs(xs[j]-spawn_model_req.initial_pose.position.x)<0.11) && (fabs(ys[j]-spawn_model_req.initial_pose.position.y)<0.1))
            {
             //spawn_model_req.initial_pose.position.y = (float)((rand()%40-10)*0.01);  // random between -0.4 to 0.4
             spawn_model_req.initial_pose.position.x -= 0.1; // random between -0.4 to 0.4
             xs[i]=spawn_model_req.initial_pose.position.x;
             ys[i]=spawn_model_req.initial_pose.position.y;
             j=0;
            }
          ...
          model_name = ros::this_node::getName()+"block_" + index;  // initialize model_name
          ...
          ros::spinOnce();
          //ros::Duration(1.0).sleep();  // frequency control, spawn one cylinder in each loop
